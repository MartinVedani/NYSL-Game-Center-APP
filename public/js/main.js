...
// NEXT MONTH's CALENDAR (Calendar2 showing)
...
function showCalendar2(month2, year2) {
    let firstDay2 = (new Date(year2, month2)).getDay();
    let daysInMonth2 = 32 - new Date(year2, month2, 32).getDate();
    let tbl2 = document.getElementById("calendar-body2"); // body of the calendar
    // clearing all previous cells
    tbl2.innerHTML = "";
    // filing data about month and in the page via DOM.
    monthAndYear2.innerHTML = months[month2] + " " + year2;
    ...
    for (let i = 0; i < 6; i++) {
        ...
        for (let j = 0; j < 7; j++) {
            if (i === 0 && j < firstDay2) {
                ...
                cell.appendChild(cellText);
                row.appendChild(cell);
            } else if (date2 > daysInMonth2) {
                break;
            } else {
                let cell = document.createElement("td");
                let cellText = document.createTextNode(date2);
                if (date2 === today.getDate() && year2 === today.getFullYear() && month2 === today.getMonth()) {
                    cell.classList.add("bg-info");
                } // color today's date
                cell.appendChild(cellText);
                row.appendChild(cell);
...
// GOOGLE FIREBASE MESSAGING DATABASE AND USER AUTHENTICATION ///
...
var app = new Vue({
    ...
    methods: {
        send: function(gameId) {
            ...
            if (this.message != '') {
                firebase.database().ref('gameMessages').push(data);
                this.message = '';
            }
        },
...
function loadGameMessages() {
    firebase.database().ref('gameMessages').on('child_added', function(childSnapshot, prevChildKey) {
        app.gameMessages.push(childSnapshot.val())
    });
};
...
firebase.auth().onAuthStateChanged(function(user) {
    if (user) {
        ...
        app.user = null;
        location.href = "/index.html"; // go back to login user interface page index.html
    }
});